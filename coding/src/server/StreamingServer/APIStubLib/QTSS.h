...
/*	QTSS对象为模块和服务器之间的数据交换提供了一种途径。QTSS对象是由一些属性组成的，这些属性用于存储数据。每个属性都有一个名称，一个属性ID，一个数据类型，以及一个读写属性值的权限。
	内置的属性是服务器在通常情况下为某个对象类型定义的属性。举例来说，QTSS_RTSPRequestObject对象有一个内置的URL属性，
	其它模块可以通过读取这个属性来得到与特定RTSP请求相关联的URL。
...
enum
{
    ...
    qtssOpenFileAsync =         1,  // 文件流将是异步的 (读操作可能会返回QTSS_WouldBlock)
    qtssOpenFileReadAhead =     2   // 文件流用于从文件线性读取.
};
...
// SERVER STATES
//
//  An attribute in the QTSS_ServerObject returns the server state as a QTSS_ServerState. Modules may also set the server state.
//
//  Setting the server state to qtssFatalErrorState, or qtssShuttingDownState will cause the server to quit.
//
//  Setting the state to qtssRefusingConnectionsState will cause the server to start refusing new connections.
enum
{
...
enum
{
    ...
    qtssCliSesReqQueryString        = 24,   //read      //char array    //Query string from the request that creates this  client session		//qtssCliSesReqQueryString 创建当前会话的客户请求的请求字符串。

    qtssCliRTSPReqRespMsg           = 25,   //read      //char array    // from the most recent (last) request. Error message sent back to client if response was an error.		//qtssCliRTSPReqRespMsg 当响应最近一次客户请求发生错误时发送给客户的错误信息。
    ...
    qtssCliSesPacketLossPercent     = 27,   //read      //Float32   //Current percent loss as a fraction. .5 = 50%. This is not an average.		//qtssCliSesPacketLossPercent 丢失数据包的百分比。例如，.5 = 50%
    qtssCliSesTimeConnectedInMsec   = 28,   //read      //SInt64    //Time in milliseconds that this client has been connected.		//qtssCliSesTimeConnectedinMsec 客户会话的连接时长，以毫秒计。
    qtssCliSesCounterID             = 29,   //read      //UInt32    //A unique, non-repeating ID for this session.		//qtssCliSesCounterID 会话的唯一ID，基于计数器机制。
...
enum
{
    ...
    qtssRTSPSesLocalAddrStr = 2,        //read      //char array            //Ditto, in dotted-decimal format.//qtssRTSPSesLocalAddrStr 当前RTSP会话的本地IP地址，以带点的十进制数格式表示。
    qtssRTSPSesLocalDNS     = 3,        //read      //char array            //DNS name of local IP address for this RTSP connection.//qtssRTSPSesLocalDNS 与当前RTSP会话的本地IP地址相关联的DNS名称。
...
enum 
{
    ...
    qtssRTSPReqFullRequest          = 0,    //read      //char array        //The full request sent by the client  	//qtssRTSPReqFullRequest 完整的，来自客户端的RTSP请求。这个属性对于所有接收到QTSS_RTSPRequestObject对象的角色都是可用的。
...
enum  
{
    ...
    qtssRTPSvrNumUDPSockets         = 12,   //read      //UInt32    //Number of UDP sockets currently being used by the server	//qtssRTPSvrNumUDPSockets 服务器当前正在使用的UDP套接口数。
    ...
    qtssSvrHandledMethods           = 20,   //r/w       //QTSS_RTSPMethod   //The methods that the server supports. Modules should append the methods they support to this attribute in their QTSS_Initialize_Role.//qtssSvrHandledMethods 服务器支持的方法。模块应该在QTSS_Initialize_Role角色中将自己支持的方法附加到这个属性上。
    ...
    qtssSvrCurrentTimeMilliseconds  = 28,   //read      //QTSS_TimeVal  //Server's current time in milliseconds. Retrieving this attribute is equivalent to calling QTSS_Milliseconds//qtssSvrCurrentTimeMilliseconds 服务器当前的时间，以毫秒计。获取这个属性的值等同于调用//QTSS_Milliseconds函数。
...
enum
{
    ...
    qtssPrefsDefaultAuthorizationRealm      = 28,   // "default_authorization_realm" //char array   //		//qtssPrefsDefaultAuthorizationRealm 这个属性指定一个文本，该文本将作为客户登录"领域（realm）"被显示。缺省情况下，这个属性的值为Streaming Server（流媒体服务器）。如果这个属性没有被设定，则显示为Streaming Server（流媒体服务器）。
...
/********************************************************************/
// QTSS API ROLES 参考 “QTSS模块角色”
//
// Each role represents a unique situation in which a module may be invoked. Modules must specify which roles they want to be invoked for. 


	/*
		角色名称							常数							任务
   =======================================================================================================
	Register（注册）				QTSS_Register_Role 				注册模块支持的角色。
	Initialize（初始化）			QTSS_Initialize_Role			执行模块的初始化任务。
	Shutdown（关闭）				QTSS_Shutdown_Role				执行清除现场任务。
	Reread Preferences				QTSS_RereadPrefs_Role	重新读取模块的预置信息。
	Error Log（错误纪录）			QTSS_ErrorLog_Role				记录错误信息。
	RTSP Filter（过滤器）			QTSS_RTSPFilter_Role			修改RTSP请求的内容。
	RTSP Route（路由）				QTSS_RTSPRoute_Role				把来自客户端的请求路由到恰当的文件夹。
	RTSP Preprocessor（预处理器）	QTSS_RTSPPreProcessor_Role		在服务器处理客户请求之前，先行进行处理。
	RTSP Request（请求）			QTSS_RTSPRequest_Role			在没有其它角色处理客户请求的情况下，对客户请求进行处理
	RTSP Postprocessor（后处理器）	QTSS_RTSPPostProcessor_Role		在请求得到响应之后，执行诸如纪录统计信息这样的任务。
	RTP Send Packets（发送数据包）	QTSS_RTPSendPackets_Role		发送数据包。
	Client Session Closing			QTSS_ClientSessionClosing_Role	在客户会话关闭时执行任务。
	RTCP Process（处理）			QTSS_RTCPProcess_Role			处理RTCP接受方的报告。
	Open File Preprocess（打开文件预处理）	QTSS_OpenFilePreProcess_Role 	处理打开文件的请求。
	Open File（打开文件）			QTSS_OpenFile_Role				处理没有被Open File Preprocess角色处理过的打开文件请求。
	Advise File（建议文件）			QTSS_AdviseFile_Role			在模块（或者服务器）调用RQTSS_Advise回调函数，获取文件对象的时候，进行响应。
	Read File（读取文件）			QTSS_ReadFile_Role				读取文件。
	Request Event File（请求事件文件）	QTSS_RequestEventFile_Role	客户端可能请求接收当文件变为可供读取时发出的通知，这个角色负责处理这些请求。
	Close File（关闭文件）			QTSS_CloseFile_Role				关闭先前打开的文件。*/
enum
{
    //Global
...
typedef QTSS_Object             QTSS_ServerObject;			// 	对应数据存贮对象为QTSServerInterface，API中标识为QTSS_ServerObject。在API中的每一个QTSS_ServerAttributes都在基类中声明和实现。
...
typedef struct
{
    char outModuleName[QTSS_MAX_MODULE_NAME_LENGTH];		//模块用于告诉Streaming Server模块的名字
} QTSS_Register_Params;

//在调用Initialize角色时，模块会接收到一个QTSS_Initialize_Params结构
typedef struct
{
    QTSS_ServerObject           inServer;           // Global dictionaries，这是一个QTSS_ServerObject对象，包含服务器的全局属性，以及一个含有当前运行的服务器的所有模块信息的属性。如果需要每个属性的描述，请参见"qtssServerObjectType"部分。
    QTSS_PrefsObject            inPrefs;			//这是一个QTSS_PrefsObject对象，包含服务器的预置信息。如果需要每个属性的描述，请参见"qtssPrefsObjectType"部分
    QTSS_TextMessagesObject     inMessages;			//这是一个QTSS_TextMessagesObject对象，模块可以用这个对象来提供本地化文本字符串。请参见"qtssTextMessageObjectType"部分。
    QTSS_ErrorLogStream         inErrorLogStream;   // Writing to this stream causes modules to be invoked in the QTSS_ErrorLog_Role
    QTSS_ModuleObject           inModule;			//这是一个QTSS_ModuleObject对象，模块可以用它来存储自身的信息，包括模块的名称，版本号，以及模块功能描述。请参见"qttsModuleObjectType"部分
} QTSS_Initialize_Params;



/*	inVerbosity: 指定这个错误信息的冗长程度。模块应该使用QTSS_Write函数中的inFlags参数来指定信息的冗长程度。QTSS定义了下面这些常数：
		qtssFatalVerbosity = 0, qtssWarningVerbosity = 1, qtssMessageVerbosity = 2, qtssAssertVerbosity = 3, qtssDebugVerbosity = 4。

	inBuffer
		指向一个以null结尾的错误信息。以qtssFatalVerbosity级别输出错误信息将会导致服务器马上关闭。想错误纪录写入信息不会导致QTSS_WouldBlock错误。
		希望自己的Error Log角色被调用的模块必须在其Register角色中调用QTSS_AddRole函数，并将角色的实参指定为QTSS_ErrorLog_Role常数。
...
inRTSPRequest:	
	这是QTSS_RTSPRequestObject对象，表示当前这个RTSP请求。在RTSP Filter角色被调用的时候，只有qtssRTSPReqFullRequest属性的值是正当的。
	如果需要RTSP请求对象属性的信息，请参见"qtssRTSPRequestObjectType"部分。

outNewRequest:	这是一个指向内存地址的指针。
	模块可以通过QTSS_GetValuePtr函数来从qtssRTSPReqFullRequest属性中获取引起服务器调用该角色的RTSP请求的全部信息。
	qtssRTSPReqFullRequest属性是一个只读属性。如果要修改RTSP请求，模块需要调用QTSS_New函数来分配一个缓冲区，
	把修改完成的请求写到缓冲区中，然后把该缓冲区的指针通过QTSS_StandardRTSP_Params结构的outNewRequest域返回给上层。
...
/*
在执行不同的角色时，其含义不同
	1.6.6.1.	QTSS_StandardRTSP_Params
	1.6.6.2.	RTSP Route
	1.6.6.3.	QTSS_StandardRTSP_Params
	1.6.6.4.	RTSP Request角色
	1.6.6.5.	RTSP Postprocessor角色
***/

typedef struct 
{

    QTSS_RTSPSessionObject      inRTSPSession;	//表示当前这个RTSP会话。如果需要RTSP会话对象属性的信息，请参见"qtssRTSPSessionObjectType"部分。
    QTSS_RTSPRequestObject      inRTSPRequest;	
		// 	RTSPRequestInterface - 这是内部数据的存储对象，在API中标识为QTSS_RTSPRequestObject，表示当前这个RTSP请求。
		//		1. 在RTSP Filter角色被调用的时候，只有qtssRTSPReqFullRequest属性的值是正当的。如果需要RTSP请求对象属性的信息，请参见"qtssRTSPRequestObjectType"部分。
		//		2. 在Route角色和随后的所有RTSP角色，所有的属性都被赋值了。在调用模块的RTSP Route角色之前，服务器会首先对请求进行解析，并在解析的过程中对QTSS_StandardRTSP_Params结构中QTSS_RTSPSessionObject和QTSS_RTSPRequestObject成员的所有属性进行赋值。
		//		3. 随后，每个属性都有值。QTSS_RTSPRequestObject对象有一个内置的URL属性，其它模块可以通过读取这个属性来得到与特定RTSP请求相关联的URL。
		//		类型为
		//			1.7.11.	qtssRTSPRequestObjectType
...
outNextPacketTime
	间隔时间，以毫秒为单位。在这个角色返回之前，模块需要设定一个合适的outNextPacketTime值，
		这个值是当前时刻和服务器再次为当前会话调用RTP Send Packets角色的时刻之间的时间间隔。
...
inFlags 
	这是一个文件打开的标志参数，指定调用QTSS_OpenFileObject 的模块是否可以处理异步的度操作（qtssOpenFileAsync），
		或者是否希望以从头到尾的顺序读取文件（qtssOpenFileReadAhead）。

inFileObject 
	这是一个QTSS对象，模块如果可以打开由inPath参数指定的文件，则会更新这个对象。 

	如果指定的文件是模块要处理的文件，则模块应该做好打开和配置文件所需要的一切必工作。
		它可以使用inFileObject对象来为该文件存储任何专门用于模块的信息。此外，模块还应该设定文件对象中qtssFlObjLenth和qtssFlObjModDate属性的值。
	...
	希望被以Open File Preprocess角色调用的模块必须在其Register角色中调用QTSS_AddRole函数，
		并指定QTSS_OpenFilePreprocess_Role作为角色参数。注册为这个角色的模块也必须处理下面这些角色，
		但是不必显式地加以注册： Advise File，Read File，Request Event File，和Close File。*/
typedef struct
{
    char*                       inPath;
...
outLenRead 
	...
	如果文件是以qtssOpenFileAsync标识打开的，则在读取数据可能导致线程阻塞的时候，模块应该返回QTSS_WouldBlock。
		否则，模块将会阻塞线程，直到得到所有的数据。当ioBuffer指向的缓冲区满，或者到达文件的尾部，
		文件系统模块应该将outLenRead参数设置为已经读取到的字节数，并返回QTSS_NoErr。
...
typedef union
{
    QTSS_Register_Params                regParams;		//只包含，被调用模块用于告诉Streaming Server模块的名字
...
//
// Every QTSS API must implement two functions: a main entrypoint, and a dispatch function. The main entrypoint 
// gets called by the server at startup to do some initialization. Your main entrypoint must follow the convention established below
//
