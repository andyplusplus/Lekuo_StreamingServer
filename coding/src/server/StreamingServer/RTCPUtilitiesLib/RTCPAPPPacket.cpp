...
Bool16 RTCPqtssPacket::ParseQTSSPacket(UInt8* inPacketBuffer, UInt32 inPacketLength)
{
    ...
    if (inPacketLength < (kRTCPPacketSizeInBytes + kQTSSDataOffset))	//  8 + 12 = 20
        return false;

    fRTCPAPPDataBuffer = inPacketBuffer+kRTCPPacketSizeInBytes;			// +8

    //figure out how many 32-bit words remain in the buffer
    UInt32 theMaxDataLen = (inPacketLength - kRTCPPacketSizeInBytes) - kQTSSDataOffset;	// - 8 - 12
    theMaxDataLen /= 4;
...
//读出Buffer中面向特定应用的数据,并根据前缀确定缓存中的数据是什么, fRTCPAPPDataBuffer
void RTCPqtssPacket::ParseAndStore()
{
    UInt8* qtssDataBuffer = fRTCPAPPDataBuffer+kQTSSDataOffset;	//+12
