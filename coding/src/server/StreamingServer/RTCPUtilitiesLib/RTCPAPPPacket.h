...
public:
    ...
    //Call this before any accessor method. Returns true if successful, false otherwise
	//读出Buffer中面向特定应用的数据,并根据前缀确定缓存中的数据是什么,
...
private:
    UInt8* fRTCPAPPDataBuffer;  //指向fReceiverPacketBuffer 
    ...
    UInt32 fReceiverBitRate;			//rrcv
    UInt32 fAverageLateMilliseconds;	//late
    UInt32 fPercentPacketsLost;			//loss
    UInt32 fAverageBufferDelayMilliseconds;		//bdly
    Bool16 fIsGettingBetter;			//:|:(
    Bool16 fIsGettingWorse;				//:|:)
    UInt32 fNumEyes;					//eyes
    UInt32 fNumEyesActive;				
    UInt32 fNumEyesPaused;
    
    //Proposed - are these there yet?
    UInt32 fTotalPacketsReceived;		//prcv
    UInt32 fTotalPacketsDropped;		//pdrp
    UInt32 fClientBufferFill;			//bufl
    UInt32 fFrameRate;					//frat
    UInt32 fExpectedFrameRate;			//xrat
    UInt32 fAudioDryCount;				//dry#
