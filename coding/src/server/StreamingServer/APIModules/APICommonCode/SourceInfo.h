...
/*    File:       SourceInfo.h
    ...
    Contains:   This object contains an interface for getting at any bit of "interesting" information regarding a content source in a format - independent manner.
                For instance, the derived object SDPSourceInfo parses an SDP file and retrieves all the SourceInfo information from that file.

				该类包含包含一些接口,用于获得任何有关内容的有用信息(以一种独立于格式的方式).
				例如,继存对象SDPSourceInfo解析SDP文件,并获得该文件的所有SourceInfo信息.
...
class SourceInfo
{
    ...
    protected:
        ...
        StreamInfo* fStreamArray;		//流数组
        UInt32      fNumStreams;		//数据库的数目
