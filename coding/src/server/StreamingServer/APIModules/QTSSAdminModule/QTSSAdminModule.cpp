...
void APITests_DEBUG()
{
    ...
    {   qtss_printf("QTSSAdminModule start tests \n");
        ...
        if (0)
        {
            ...
            err = QTSS_GetNumValues (sServer, qtssSvrClientSessions, &numValues);//qtssSvrClientSessions 一个对象，包含存储在索引过的QTSS_ClientSessionObject对象中的所有客户会话。
            err = QTSS_GetValuePtr(sServer, qtssSvrClientSessions, 0, (void**)&sessionsPtr, &paramLen);//qtssSvrClientSessions 一个对象，包含存储在索引过的QTSS_ClientSessionObject对象中的所有客户会话。
        ...
        if (0)
        {
            ...
            err = QTSS_GetNumValues (sServer, qtssSvrClientSessions, &numValues);			//qtssSvrClientSessions 一个对象，包含存储在索引过的QTSS_ClientSessionObject对象中的所有客户会话。
            err = QTSS_GetValue(sServer, qtssSvrClientSessions, 0, (void*)&sessions, &paramLen);		//qtssSvrClientSessions 一个对象，包含存储在索引过的QTSS_ClientSessionObject对象中的所有客户会话。
