...
Bool16 ElementNode::GetFilteredAttributeID(char *parentName, char *nodeName, QTSS_AttributeID* foundID)
{
    ...
    {
        if (0 == strcmp("qtssSvrClientSessions", nodeName) )//qtssSvrClientSessions 一个对象，包含存储在索引过的QTSS_ClientSessionObject对象中的所有客户会话。
        {   if (foundID)
                *foundID = qtssCliSesCounterID;
            found = true;
        }
        
        if (0 == strcmp("qtssSvrModuleObjects", nodeName))		//qtssSvrModuleObjects 模块对象，代表每个模块。
        {   if (foundID) 
                *foundID = qtssModName;
            found = true;
        }
