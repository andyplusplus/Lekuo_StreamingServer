        ...
        switch (fieldNum)
        {
            ...
            */
            ...
            case 26:
            {
                ...
                {
                    QTSS_Error theErr = QTSS_GetValuePtr(sServer, qtssSvrRTSPPorts, theIndex, (void**)&thePort.Ptr, &thePort.Len);  //qtssSvrRTSPPorts 这是一个索引过的属性，含有服务器正在侦听的所有端口
            ...
            case 51:
            {
                (void)QTSS_GetValueAsString(sServer, qtssRTPSvrNumUDPSockets, 0, &thePrefStr);		//qtssRTPSvrNumUDPSockets 服务器当前正在使用的UDP套接口数。
