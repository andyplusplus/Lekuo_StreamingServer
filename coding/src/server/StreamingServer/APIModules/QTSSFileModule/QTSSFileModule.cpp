    ...
    {   
        ...
        if ((*theFile)->fPacketStruct.packetData == NULL)
        {
           ...
           theLastPacketTrack = (*theFile)->fFile.GetLastPacketTrack();
            ...
            theStream = (QTSS_Object)theLastPacketTrack->Cookie1;	//得到RTPStream对象
		...
		//By Wuzengde
		//WuZengDe_PrintPacketData((BYTE*) ((*theFile)->fPacketStruct).packetData, (*theFile)->fNextPacketLen);
        ...
        { 
            ...
            if ((*theFile)->fPacketStruct.suggestedWakeupTime == -1)
                inParams->outNextPacketTime = sFlowControlProbeInterval;    //10Msec以后再次调用该函数 for buffering, try me again in # MSec
            else
            {
                Assert((*theFile)->fPacketStruct.suggestedWakeupTime > inParams->inCurrentTime);
                inParams->outNextPacketTime = (*theFile)->fPacketStruct.suggestedWakeupTime - inParams->inCurrentTime;		//下一个包的发包时间
            }
