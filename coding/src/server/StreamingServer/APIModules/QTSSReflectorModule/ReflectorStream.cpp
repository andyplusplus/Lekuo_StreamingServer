...
#if DEBUG
#define REFLECTOR_STREAM_DEBUGGING 1
#else
#define REFLECTOR_STREAM_DEBUGGING 1
#endif
					...
					if ( !dodBookmarkPacket )
					{
						...
						{	
							...
							if ( timeToSendPacket == -1 )
								fNextTimeToRun = 1; // keep in synch with delay on would block for on-demand lower is better for high-bit rate movies.
						}
					}
					else
					{	
    ...
    {                   
        ...
        { // call us again in # ms to retry on an EAGAIN
            if ((timeToSendPacket > 0) && (fNextTimeToRun > timeToSendPacket ))
                fNextTimeToRun = timeToSendPacket;
            if ( timeToSendPacket == -1 )
                fNextTimeToRun = 1; // fixes OSX (cpu spike from 100ms sample time is os) and smaller is better to avoid synch problems
