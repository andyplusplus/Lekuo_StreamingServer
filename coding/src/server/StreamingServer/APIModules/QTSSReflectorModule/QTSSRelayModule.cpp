 ...
 */
/*
    File:       QTSSRelayModule.cpp    QTSSReflectorModule.cpp
...
StrPtrLenDel sDefaultRelayPrefs_Del;	//New
static char* sDefaultRelayPrefs = "";	//DEFAULTPATHS_ETC_DIR "relayconfig.xml";
//	_WZD_PATH_ End
...
QTSS_Error  QTSSRelayModuleDispatch(QTSS_Role inRole, QTSS_RoleParamPtr inParams)
{
	...
	theRoleName[4] = NULL;
	qtss_printf(" ********** QTSSRelayModuleDispatch(QTSS_Role = %s)\n", theRoleName);
	//_WZD_ADD_End
...
QTSS_Error SetupAnnouncedSessions(QTSS_StandardRTSP_Params* inParams)
{
    ...
    theErr = QTSS_GetValue(sServer, qtssSvrRTSPPorts, 0, (void *) &thePort, &theLen);		//qtssSvrRTSPPorts 这是一个索引过的属性，含有服务器正在侦听的所有端口
...
void ReadRelayPrefsFile()
{
    ...
    if (::stat(sRelayPrefs, &buf) >= 0)
    {
        ...
        sRelayPrefModDate = static_cast <int> (buf.st_mtime);
    }
