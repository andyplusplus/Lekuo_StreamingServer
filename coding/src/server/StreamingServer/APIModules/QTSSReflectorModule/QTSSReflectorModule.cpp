...
{
	char theRoleName[5];
	QTSS_Role theOutPutRole = htonl(inRole);
	memcpy(theRoleName, &theOutPutRole, 4);
	theRoleName[4] = NULL;
	//qtss_printf(" ---------- QTSSReflectorModuleDispatch(QTSS_Role = %s)\n", theRoleName);
...
QTSS_Error Initialize(QTSS_Initialize_Params* inParams)
{
	...
	//QTSS_REFLECTOR_EXTERNAL_MODULE  is 0 is this case
...
//_WZD_ADD_Start
void Print_ReflactorTrace(int nMethod)
{

	char* theMethods[] = {	
			"DescribeMethod			QTSS_Error DoDescribe", 
			"SetupMethod			QTSS_Error DoSetup", 
			"TeardownMethod			QTSS_Error  QTSS_Teardown", 
			"PlayMethod				QTSS_Error DoPlay", 
			"PauseMethod			QTSS_Error QTSS_Pause", 
			"OptionsMethod", 
			"AnnounceMethod			QTSS_Error DoAnnounce", 
			"GetParameterMethod", 
			"SetParameterMethod", 
			"RedirectMethod", 
			"RecordMethod			QTSS_Error DoPlay", 
			"IllegalMethod"
	};
	
	if ( (nMethod < 12) && (nMethod > -1)  )
		qtss_printf("      QTSS_Error ProcessRTSPRequest		%s		called\n", theMethods[nMethod]);
...
//_WZD_ADD_Start
	qtss_printf("  +++++++++++++++     ");
	Print_ReflactorTrace(*theMethod);
//_WZD_ADD_Start
