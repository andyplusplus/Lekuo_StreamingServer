...
OSPtrDeleter<OSMutex> sLogMutex_Del(NULL);
//   _WZD_MEM_End
...
class QTSSAccessLog : public QTSSRollingLog
{
    public:
        ...
        QTSSAccessLog() : QTSSRollingLog() { 
			this->SetTaskName("QTSSAccessLog");  
		}
...
QTSS_Error Register(QTSS_Register_Params* inParams)
{
    sLogMutex = NEW OSMutex();
		...
		//   _WZD_MEM_Start
			sLogMutex_Del.SetObject(sLogMutex);
		//   _WZD_MEM_End
...
QTSS_Error Shutdown()
{
    //WriteShutdownMessage();		//_WZD_Comment_  2008.06.28  由于执行该函数前已经关闭线程及删除所有任务，包括该函数内的sAccessLog对象
