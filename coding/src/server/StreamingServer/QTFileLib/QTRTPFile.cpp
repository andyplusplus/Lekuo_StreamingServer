...
//该函数由文件名const char * filePath新建QTFile ** theQTFile, 首先在缓存中查找,如果找到该文件,则直接返回,否则新建一个文件
QTRTPFile::ErrorCode QTRTPFile::new_QTFile(const char * filePath, QTFile ** theQTFile, Bool16 debugFlag, Bool16 deepDebugFlag)
{
  ...
  //1. 首先在缓存中查找该文件, 并增加引用计数, 如果找到该文件, 则直接返回
    //
 ...
 //2. 如果找不到该文件, 新建立一个QTFile文件, 打开该文件, 并把该文件加入Cache
    //
    ...
    *theQTFile = NEW QTFile(debugFlag, deepDebugFlag);
    ...
    //
    // Open the specified movie. 打开指定的文件, 并填入相关原子
...
//该函数定义并填入newListEntry, 并把它加入到列表QTRTPFile::gFirstFileCacheEntry中
...
//从文件缓存中找到和 *inFilename 名字相同的引用, 
//如果找到, 增加引用计数, 则通过cacheEntry返回对该文件的引用, 并返回TRUE, 否则返回False
...
QTRTPFile::ErrorCode QTRTPFile::Initialize(const char * filePath)
{
    ...
    //以下代码查找文件的下一个track, 如果找到该track, 则新建track结构,并在结构中填入,数据,然后把track加入列表
...
//构造SDP文件, 具体如下
/*1.	设置sdpRangeLine，如a=range:npt=0-  70.00000，其中70.00000从QTFile的QTAtom_mvhd类计算得到，=fDuration/fTimeScale
2.	执行的循环操作，curEntry从第一个track到最后一个track，计算track的长度总和fSDPFileLength
     curEntry->HintTrack->GetSDPFileLength(&trackSDPLength)
3.	确定是否有全局SDP原子，如果有读取该原子，并加到trackSDPLength变量上
