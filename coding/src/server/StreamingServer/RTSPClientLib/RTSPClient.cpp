    ...
    fControlID(RTSPClient::sControlID)
{
    ...
    fChannelTrackMap = NEW ChannelMapElem[kMinNumChannelElements];		//如果我们interleaving，该数据结构从channel number映射到track ID
...
//QTSSProxyModule.cpp的DoRequestPreProcessing(...)函数调用以下函数
//调用形式:eturn inProxyClientInfo->GetRTSPClient()->SendRTSPRequest(theNewRequest, theNumVecs);
//tobe
...
OS_Error RTSPClient::PutMediaPacket(UInt32 inTrackID, Bool16 isRTCP, char* inData, UInt16 inLen)
{
    ...
    for (int x = 0; x < fNumChannelElements; x++)
    {
        if ((fChannelTrackMap[x].fTrackID == inTrackID) && (fChannelTrackMap[x].fIsRTCP == isRTCP))
        {
            ...
            // Build the iovec
            iovec ioVec[2];
            ioVec[0].iov_len = 4;					//前面存放头的长度
            ioVec[0].iov_base = header;
            ioVec[1].iov_len = inLen;				//后面存放数据
...
OS_Error    RTSPClient::GetMediaPacket(UInt32* outTrackID, Bool16* outIsRTCP, char** outBuffer, UInt32* outLen)
{
    ...
    static const UInt32 kMaxPacketSize = 8192;
    ...
    {   // 对该函数的前一次调用已经获得了数据包，现在我们可以把数据包移去
        UInt16* thePacketLenP = (UInt16*)fPacketBuffer;	//指向包的长度
    ...
    {
        // 如果header buffer有数据，确保数据的合法性, 把数据包头copy到fPacketBuffer
	...
	// 如果已经fPacketBufferOffset > 4
    if (fPacketBufferOffset > kPacketHeaderLen)
    {
        Assert(fPacketBuffer[0] == '$');
        UInt16* thePacketLenP = (UInt16*)fPacketBuffer;
        UInt16 thePacketLen = ntohs(thePacketLenP[1]); //读出包长
        
        if (fPacketBufferOffset >= (thePacketLen + kPacketHeaderLen))	//表明我们有一个完整的包，把包发给客户端
        {
            ...
            *outLen = thePacketLen;
            ...
            // Next time we call this function, we will blow away the packet, but until then we leave it untouched. 
			// 下次调用该函数时，我们需要请除当前包
...
OS_Error RTSPClient::DoTransaction()
{
    ...
    if (!fTransactionStarted)		//接下去要启动Transaction
    {   
        //
    ...
    {//以下函数发送数据，并设置相关状态
        ...
        fHaveTransactionBuffer = false;		//发送完
    ...
    if (fReceiveInProgress)
    {//以下函数接收数据，如果接收数据，只能接收一个备份?
     //如果这样的话，函数的实现还有些问题
        Assert(theErr == OS_NoErr);
        theErr = this->ReceiveResponse();  //能否改一个状态，使程序能接收多个执行状态
    ...
    {//如果状态是"401"--Unauthorized，则发送认证信息
        fResponseCount ++;
...
OS_Error RTSPClient::ReceiveResponse()
{
  ...
  //接收并解析回应
	...
	//2.确保接收到了正确的头信息
        ...
        else if (theHeaderStart != fRecvHeaderBuffer)  //如果RTSP没有在内存区的头部，则移动内存块
        {
        ...
        if (theResponseData != NULL)  //如果Interleaved了回应数据
        {               
            // skip past the \r\n\r\n
            theResponseData += 4;
			...
			//Session读取
                if (theKey.NumEqualIgnoreCase(sSessionHeader.Ptr, sSessionHeader.Len))
                {
                    if (fSessionID.Len == 0)
                    {   //读取会话ID
                        // Copy the session ID and store it. First figure out how big the session ID is. 
						// We copy everything up until the first ';' returned from the server
                        UInt32 keyLen = 0;
...
//解析RTP信息头
//RTP-Info: url=rtsp://159.99.242.108/sample_100kbit.mov/trackID=3;seq=24102;rtptime=24356,
//          url=rtsp://159.99.242.108/sample_100kbit.mov/trackID=4;seq=1335;rtptime=12124
void    RTSPClient::ParseRTPInfoHeader(StrPtrLen* inHeader)
{
    ...
    while (theRTPInfoSubHeader.Len > 0)
    {
        ...
        else if (theRTPInfoSubHeader.NumEqualIgnoreCase(sSSRCSubHeader.Ptr, sSSRCSubHeader.Len))
        {
            StringParser theURLParser(&theRTPInfoSubHeader);
            theURLParser.ConsumeUntil(NULL, StringParser::sDigitMask);
            fSSRCMap[fNumSSRCElements].fSSRC = theURLParser.ConsumeInteger(NULL);  //tobe程序有问题
        }
