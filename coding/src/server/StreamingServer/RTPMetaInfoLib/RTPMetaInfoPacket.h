...
//定义了RTP-Meta-Info的Payloads, 以及从缓存解析RTP包，并可按照类中的值重新生成RTP包, 正常的播放过程不调用该类中的函数
// RTPMetaInfoPacket.h:
...
class RTPMetaInfoPacket
{
    public:
        ...
        // 从x-RTP-Meta-Info头构造标准的FieldID数组，从Buffer中的数据构造ID数组
        ...
        // 解析包
        // Call this to parse the RTP-Meta-Info packet. Pass in an array of FieldIDs, make sure it is kNumFields in length.
        // This function will use the array as a guide to tell which field IDs in the packet refer to which fields.
        Bool16  ParsePacket(UInt8* inPacketBuffer, UInt32 inPacketLen, FieldID* inFieldIDArray);
        
        // 生成RTP包，剥离RTP扩展字段，该操作将覆盖Buffer中的数据，正常的RTP头为12个字节，该操作该把这12个字节去掉
        // Call this if you would like to rewrite the Meta-Info packet as a normal RTP packet (strip off the extensions). Note that
        // this will overwrite data in the buffer! Returns a pointer to the new RTP packet, and its length
    ...
    private:
        ...
        static const FieldName kFieldNameMap[];		//整形数组
