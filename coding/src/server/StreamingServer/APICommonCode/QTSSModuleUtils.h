...
class QTSSModuleUtils
{
    public:
		...
		/*    
			QTSS_TextMessagesObject   inMessages,			//文本消息对象
			QTSS_ServerObject   inServer,					//服务器对象
			QTSS_StreamRef   inErrorLog					//流引用对象，指向错误的引用
		*/
		...
		// 如果你的模块支持RTSP方法, QTSS_Initialize调用该函数, 告诉服务器这些方法是什么, 实际上是调用QTSS_SetValue(...)
...
class IPComponentStr
{
    ...
    StrPtrLen   fAddressComponent[kNumComponents];	//把IP地址的各个组件存入fAddressComponent[...]
    Bool16      fIsValid;							//是否是合法的IP地址
    static IPComponentStr sLocalIPCompStr;			//本地IP地址,设置为 127.0.0.*
