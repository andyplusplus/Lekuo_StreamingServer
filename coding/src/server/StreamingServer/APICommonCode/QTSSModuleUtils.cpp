...
void    QTSSModuleUtils::SetupSupportedMethods(QTSS_Object inServer, QTSS_RTSPMethod* inMethodArray, UInt32 inNumMethods)
{
    ...
    (void)QTSS_GetNumValues(inServer, qtssSvrHandledMethods, &theNumMethods);//qtssSvrHandledMethods 服务器支持的方法。模块应该在QTSS_Initialize_Role角色中将自己支持的方法附加到这个属性上。
    
    for (UInt32 x = 0; x < inNumMethods; x++)
        (void)QTSS_SetValue(inServer, qtssSvrHandledMethods, theNumMethods++, (void*)&inMethodArray[x], sizeof(inMethodArray[x]));//qtssSvrHandledMethods 服务器支持的方法。模块应该在QTSS_Initialize_Role角色中将自己支持的方法附加到这个属性上。
}
...
QTSS_ModulePrefsObject QTSSModuleUtils::GetModuleObjectByName(const StrPtrLen& inModuleName)
{
    ...
    for (int x = 0; QTSS_GetValue(sServer, qtssSvrModuleObjects, x, &theModule, &theLen) == QTSS_NoErr; x++)//qtssSvrModuleObjects 模块对象，代表每个模块。
    {
