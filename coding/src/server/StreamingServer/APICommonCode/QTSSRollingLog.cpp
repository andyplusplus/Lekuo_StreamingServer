...
static Bool16 sCloseOnWrite = true;		//如果为true, 则每次写日志以后, 关闭文件
...
//把数据写入日志, allowLogToRoll: log是否要roll
void QTSSRollingLog::WriteToLog(char* inLogData, Bool16 allowLogToRoll)
{
    ...
    if (fLog != NULL)
    {
        qtss_fprintf(fLog, "%s", inLogData);	//把数据写入日志
        ::fflush(fLog);
    }
