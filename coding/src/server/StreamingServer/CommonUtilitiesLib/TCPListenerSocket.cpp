...
{
    //we are executing on the same thread as every other socket, so whatever you do here has to be fast.
...
//test acceptError = ENOENT;
    ...
    {   
        ...
        theSocket->RequestEvent(EV_RE);		//RTSP时，调用EventContext::RequestEvent(int theMask)，EventContext添加到EventThread的引用表中
