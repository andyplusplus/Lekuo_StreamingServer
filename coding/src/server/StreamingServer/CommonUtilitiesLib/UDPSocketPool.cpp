...
UDPSocketPair*  UDPSocketPool::CreateUDPSocketPair(UInt32 inAddr, UInt16 inPort)
{
    ...
    Assert(theElem != NULL);
    if (theElem->fSocketA->Open() != OS_NoErr)	//创建socket, -->Socket::Open(SOCK_DGRAM)-->::socket(PF_INET, theType, 0);/this->InitNonBlocking(fFileDesc);
    {
        this->DestructUDPSocketPair(theElem);
        return NULL;
    }
    if (theElem->fSocketB->Open() != OS_NoErr)	//创建socket
    {
        this->DestructUDPSocketPair(theElem);
        return NULL;
    }
    ...
    this->SetUDPSocketOptions(theElem);			//设置缓存大小
    ...
    UInt16 startIndex = kLowestUDPPort;		//设置端口的范围
    if (inPort != 0)
	{
        startIndex = inPort;
...
//#endif
    ...
    while ((!foundPair) && (startIndex <= kHighestUDPPort))
    {
        ...
        if (theErr == OS_NoErr)
        {
            ...
            {
                foundPair = true;
                fUDPQueue.EnQueue(&theElem->fElem);	//把Socket对加入UDPSocketPool.fUDPQueue队列
                theElem->fRefCount++;
                return theElem;
            }
        }
