...
class OSMutexRW
{
    ...
    private:
      ...
      //说明: 每执行一次LockRead(),    State += 1;  fActiveReaders = state;
	  //        执行一次LockWrite()后, State = -1;  fActiveReaders = state;
...
class   OSMutexReadWriteLocker
{
    public:
        ...
        ~OSMutexReadWriteLocker() { if (fRWMutexPtr != NULL) fRWMutexPtr->Unlock(); }		//析构时解锁


        void UnLock() { if (fRWMutexPtr != NULL) fRWMutexPtr->Unlock(); }					//解锁
