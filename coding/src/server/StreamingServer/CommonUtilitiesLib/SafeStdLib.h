...
#include "myPrintf.h"   //Added by WuZengDe, 2007.11.21 把输出结果打钱到Windows Debug窗口
...
#define qtss_fprintf fprintf
	...
	#define qtss_printf qtss_maxprintf
#else
//myPrintf  Added by WuZengDe, 2007.11.21 把输出结果打钱到Windows Debug窗口

	#define _MY_PRINTF_Nothing 0

	#if _MY_PRINTF_Nothing
		#ifdef _MY_PRINTF_
			#define qtss_printf myPrintf_Nothing
		#else
			#define qtss_printf printf_Nothing
		#endif
	#else
		#ifdef _MY_PRINTF_
			#define qtss_printf myPrintf
		#else
