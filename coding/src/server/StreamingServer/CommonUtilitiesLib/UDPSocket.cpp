...
UDPSocket::SendTo(UInt32 inRemoteAddr, UInt16 inRemotePort, void* inBuffer, UInt32 inLength)
{
	...
	//_WZD_DEBUG_START
	//char thePrintC [1024];
	//sprintf(thePrintC, "UDPSocket::SendTo(fFileDesc = %d, inLength = %d, theErr = %d\n", fFileDesc, inLength, theErr);
	//OutputDebugString(thePrintC);
	//_WZD_DEBUG_END
...
//_WZD_ADD_Start
static long RecvTrapIn = 0; 
//_WZD_ADD_End


//receive data from remote address, and write the address
                            ...
                            void* ioBuffer, UInt32 inBufLen, UInt32* outRecvLen)
{
	...
	{
		++RecvTrapIn;
		theRecvLen = ::recvfrom(fFileDesc, (char*)ioBuffer, inBufLen, 0, (sockaddr*)&fMsgAddr, &addrLen);
		//qtss_printf("			UDPSocket::RecvFrom   theRecvLen = %d,   fFileDesc = %d\n", theRecvLen, fFileDesc);
		--RecvTrapIn;
	}
#endif
