...
//从当前类的字符串中，查找字符串的结果，并把找到的结果字符串通过resultStr返回
//在这个过程中，没有申请新的内存，只是把返回的结果的指针通过resultStr返回
char *StrPtrLen::FindStringCase(char *queryCharStr, StrPtrLen *resultStr, Bool16 caseSensitive) const
{
    ...
    char *dupSourceString = NULL;			//复制了的字符串，用于存贮大写的字符信息
    char *dupQueryString = NULL;
    char *sourceString = Ptr;				//指向源字符串
    UInt32 foundLen = 0;
    ...
    {   foundLen = resultChar - sourceString;		//resultChar所在的位置
