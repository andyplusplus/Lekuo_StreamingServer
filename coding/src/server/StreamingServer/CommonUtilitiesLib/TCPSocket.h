...
class TCPSocket : public Socket
{
    public:
		...
		//TCPSocket携带一个task object，当socket发生某些事件时，会通知该task object，事件有:
		//S_DATA:               socket 有数据
		//S_CONNECTIONCLOSING:客户正关闭连接，不再需要调用Close或Disconnect, Snd及Rcv会失败
        ...
        // Connect. 尝试连接到特定的远程主机，如果是非阻塞socket，该函数将返回EINPROGRESS, in which
        // 此时调用者必须等待EV_RE或EV_WR。你可随时调用CheckAsyncConnect，如果连接完成，
        // 函数将返回OS_NoErr, 如果还在连接返回EINPROGRESS, 如果连接失败返回一个合适的错误
