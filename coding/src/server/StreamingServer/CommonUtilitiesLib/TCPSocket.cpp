...
//2. 在不关闭其它socket的情况下，获得其它socket，把目标socket的fFileDesc设空，fState = 0
...
OS_Error  TCPSocket::Connect(UInt32 inRemoteAddr, UInt16 inRemotePort)
{
    ...
    int err = ::connect(fFileDesc, (sockaddr *)&fRemoteAddr, sizeof(fRemoteAddr));	//建立与特定socket的连接
    fState |= kConnected;
