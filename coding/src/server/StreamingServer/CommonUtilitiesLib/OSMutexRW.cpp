...
void OSMutexRW::LockRead()
{
    ...
    AddReadWaiter();						//fReadWaiters += 1
	//如果当有人在写, 则等待写的人写完
    while   (   ActiveWriter() // active writer so wait						//fState < 0
            ||  WaitingWriters() // reader must wait for write waiters		//fWriteWaiters > 0
            )
    {   //如果此前有work写过了,则
        fReadersCond.Wait(&fInternalLock,OSMutexRW::eMaxWait);	//一直等待
    }
        
    RemoveReadWaiter();					//fReadWaiters -= 1
    AddActiveReader();					//State = State+ 1
    fActiveReaders = fState;			//fActiveReaders = state
...
{
    OSMutexLocker locker(&fInternalLock);
    AddWriteWaiter();       //  1 writer queued   fWriteWaiters += 1
#if DEBUGMUTEXRW
    ...
    while   (ActiveReaders())  // active readers  //fState > 0 
    { //如果说有Read数,则执行
        fWritersCond.Wait(&fInternalLock,OSMutexRW::eMaxWait);
    }

    RemoveWriteWaiter(); // remove from waiting writers
    SetState(OSMutexRW::eActiveWriterState);    // this is the active writer   // State = -1
    fActiveReaders = fState;													// fActiveReaders = state
#if DEBUGMUTEXRW
...
//  qtss_printf("Unlock active readers = %d, waiting writers = %d, waiting readers=%d\n", fActiveReaders, fReadWaiters, fWriteWaiters);
    ...
    if (ActiveWriter())		
    {           
        SetState(OSMutexRW::eNoWriterState); // fWriteWaiters > 0 // this was the active writer 
