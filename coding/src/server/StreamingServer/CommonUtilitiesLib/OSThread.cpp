...
#ifdef __Win32__
DWORD   OSThread::sThreadStorageIndex = 0;		//该变量用来存贮当前线程的指针
#elif __PTHREADS__
...
{
#ifdef __Win32__
    sThreadStorageIndex = ::TlsAlloc();		//分配存贮索引，该变量是一个静态变量
...
void OSThread::UnInitialize()
{
	...
	{
		int theResult = TlsFree(sThreadStorageIndex);
		Assert(theResult);
...
void OSThread::Join() //等待线程运行完成后删除
{
...
#endif
{
...
#ifdef __Win32__
    BOOL theErr = ::TlsSetValue(sThreadStorageIndex, theThread);	//设置存贮索引，该线程的数据将存到设置的存贮索引里
    Assert(theErr == TRUE);
#elif __PTHREADS__
    ...
    //
    // Run the thread
    theThread->Entry();		//由于是纯虚函数,所以调用的是子类的Entry()函数
    return NULL;
}
...
{
#ifdef __Win32__
    return (OSThread *)::TlsGetValue(sThreadStorageIndex);   //sThreadStorageIndex变量用来存贮当前线程的指针
#elif __PTHREADS__
