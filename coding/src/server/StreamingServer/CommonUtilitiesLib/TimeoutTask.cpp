...
SInt64 TimeoutTaskThread::Run()
{
	...
	SInt64 intervalMilli = kIntervalSeconds * 1000;//默认总为60秒，但最小值 > 0  //always default to 60 seconds but adjust to smallest interval > 0
    ...
    for (OSQueueIter iter(&fQueue); !iter.IsDone(); iter.Next())
    {
        ...
        //如果任务需要超时，则触发超时          //if it's time to time this task out, signal it
