...
//该函数主要目的是把IP地址列表存入sIPAddrInfoArray数组, 该函数执行的功能如下:
...
{
	IPAddrInfo theIPAddrInfo;

	for(UInt32 i = 0; i < sNumIPAddrs; i++ )
	{
		theIPAddrInfo = sIPAddrInfoArray[i];

		if (theIPAddrInfo.fIPAddrStr.Ptr)
		{
			theIPAddrInfo.fIPAddrStr.Delete();
		}

		if (theIPAddrInfo.fDNSNameStr.Ptr)
		{
			theIPAddrInfo.fDNSNameStr.Delete();
		}
...
/*
参数: Bool16 lookupDNSName: 参数: 是否查找DNS名字，初始化时参数值为False
该函数主要目的是把IP地址列表存入sIPAddrInfoArray数组
该函数执行的功能如下:
1.	首先创建socket
2.	通过WSAIoctl(…)函数获得IP列表addrListP，
a)	sNumIPAddrs为IP地址数
b)	IP地址存入sIPAddrInfoArray数组成员的fIPAddr和fIPAddrStr元素中，数组成员为IPAddrInfo类型
c)	通过gethostbyaddr(…)函数把IP地址转化为DNS地址，把DNS地址存入sIPAddrInfoArray数组成员的fDNSNameStr元素，数组成员为IPAddrInfo类型
        struct IPAddrInfo
        {
            UInt32      fIPAddr;
