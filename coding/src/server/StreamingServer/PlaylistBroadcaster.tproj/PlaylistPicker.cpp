...
/*    PlaylistPicker has 3 modes
        ...
        - sequential looping through the items in the play list(s) in the order they are entered.
...
PlaylistPicker::PlaylistPicker( UInt32 numBuckets, UInt32 numNoRepeats )
{
    ...
    mRecentMoviesListSize = numNoRepeats;		//双向列表mElementLists的最大长度
...
/* ***************************************************** */
    ...
    mUsedElements = new NoRepeat( numNoRepeats );	//只是申请一个PLDoubleLinkedList对象, 该对象最多有numNoRepeats非重复的结点

    Assert( mUsedElements );
}

//构造函数, 表示是否是SequentialLooping(即是否循环选取节目), 默认mBuckets= 2, 默认顺序选取, 不定义NoRepeat对象
...
//按照一个的选取策略选择一个节目, 具体执行过程如下: 
char* PlaylistPicker::PickOne()
{
    //该函数设置foundName, 并返回foundName的名字, 指向我们选中的节目名, 由调用者删除
    char*   foundName = NULL;   // pointer to name of pick we find, caller deletes.
    
//1. 如果是顺序选取节目
    if ( mIsSequentialPicker ) 
    {   
		//1.1 如果一个桶没有表项, 并且是顺序循环选节目 , 切换到另一个桶. (其实切换到另一个桶即是表示循环过程)
        if ( mElementLists[mWhichSequentialBucket]->GetNumNodes() == 0 && mIsSequentialLooping ) //如果一个桶中没有元素, 并且是顺序选取节目
		...
		//1.2 如果桶中有一定数量的结点, 从双向列表(桶)中选出第一个结点, 删除该节点, 并加入另一个双向列表(桶)的尾部
        if ( mElementLists[mWhichSequentialBucket]->GetNumNodes() > 0 )
        {
            ...
            Assert( foundName );
            //如果找到相应的结点, 如果mRemoveFlag, mNumToPickFrom--. 否则把找到的结点加入另一个双向列表的尾部
            if ( foundName )
            {   
                int usedBucketIndex;
