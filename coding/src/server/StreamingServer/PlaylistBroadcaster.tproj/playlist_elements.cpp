...
#ifdef __Win32__
    if (fSocketRTp != 0) ::closesocket(fSocketRTp);	//该函数关闭现有的socket
...
//重新打开UDPSocketPair中的RTP以及RTCPSocket
...
//初始化fLocalAddrRTp, fLocalAddrRTCp, fDestAddrRTp, fDestAddrRTCp为0
...
//把socket和特定的地址绑定, 这样就可以开始侦听了. 主要处理本地地址和端口
//该函数使用了for循环,如果某一端口已经初使用, 则绑定失败, 自动跳到下一个端口.
SInt16 UDPSocketPair::Bind(UInt32 addr)
{
    ...
    InitPorts(addr);	//初始化fLocalAddrRTp, fLocalAddrRTCp, fDestAddrRTp, fDestAddrRTCp为0
        
    UInt32 PortRTp = eSourcePortStart;  //49152
...
//设置fDestAddrRTp, 以及fDestAddrRTCp
...
//打开并绑定UDPSocketPair端口对, 然后设置目标地址
SInt16 UDPSocketPair::OpenAndBind( UInt16 rtpPort,UInt16 rtcpPort,char *destAddress) 
{
    ...
    do
    {
        ...
        err = this->Bind(INADDR_ANY);	//绑定本地地址和端口
        if (err != 0) break;
        
        err = this->SetDestination (destAddress, rtpPort, rtcpPort);    //远程地址和端口
        if (err != 0) break;
