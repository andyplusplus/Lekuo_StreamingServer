...
//
// ************************
//为每个媒体流MediaStream设置一个唯一的SSRC, SSRC随机生成
...
//启动所有的流, 调用所有MediaStream的StreamStart(startTime);
...
//启动Movie, 调用所有MediaStream的MovieStart(startTime)
...
//结束Movie, 调用所有MediaStream的MovieEnd(endTime)
...
//调用所有MediaStream的ReceiveOnPorts()
...
//调用所有MediaStream的UpdateSenderReport(theTime);
