...
//1. 该函数初设置为回调函数, 由int ParseConfigFile(...)来调用
//2. PLBroadcastDef::PLBroadcastDef(...)在调用ParseConfigFile时, 把该函数作为参数传入
//3. 函数的目的为: 把paramValue值设置给相应的变量
Bool16 PLBroadcastDef::ConfigSetter( const char* paramName, const char* paramValue[], void* userData )
{
    ...
    */
	...
	else if (!::strcmp( "broadcast_start_time", paramName) )
	{
        ...
        if ('*' == theValue[0])
	    {    
	        time_t startTime =  time(NULL) + 2208988800LU + (time_t) ::strtol(&theValue[1], NULL, 10);
    ...
    return true;
}//Bool16 PLBroadcastDef::ConfigSetter(

//为*dest申请内存, 并把*value 拷贝到该内存
Bool16 PLBroadcastDef::SetValue( char** dest, const char* value)
{
    ...
    *dest = new char[ strlen(value) + 1 ];
    ...
    return didFail;
}//Bool16 PLBroadcastDef::SetValue(


//设置相关属性的默认值, 函数的实现比较简单
...
/* ***************************************************** */
    return didFail;
}//Bool16 PLBroadcastDef::SetDefaults(
    ...
    
{
    ...
    if (setupFileName )
    {
        ...
        {   err = ::ParseConfigFile( false, setupFileName, ConfigSetter, this );  //解析PlayList100K.config
        }
...
void PLBroadcastDef::ValidateSettings()
{
    ...
    if ( mInvalidParamFlags | kInvalidNone )
        mParamsAreValid = false;
}//PLBroadcastDef::ValidateSettings()
