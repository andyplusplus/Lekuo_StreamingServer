...
class BroadcastLog : public QTSSRollingLog
{
    ...
    public:
        ...
        void    LogMediaError( const char* path, const char* errStr, const char* messageStr);	//日志错误信息
        ...
        void    LogMediaData(	const char* path, const char* title, const char* copyright, 
                                ...
                                const char* album, UInt32 duration, SInt16 result);				//日志媒体数据
    ...
    protected:
        char    mDirPath[256];		//日志目录路径
        char    mLogFileName[256];	//日志文件名字
        bool    mWantsLogging;		//是否需要写日志
