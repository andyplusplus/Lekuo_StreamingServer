...
//有效载荷的标签信息, 由SDP的a=rtpmap:96 X-SV3V-ES/90000  解析得到
class PayLoad {
    public:
    PayLoad(void) : payloadID(0), timeScale(1) {};
    int             payloadID;		//97
    SimpleString    payLoadString;	//X-SV3V-ES
    UInt32          timeScale;		//90000, 默认为1
};

//包含指向MediaStream对象指针以及一些标签信息的数据结构
//该数据结构的值SDP文件解析得到
class TypeMap {
    public:
        ...
        MediaStream                 *fMediaStreamPtr;	//包含一个或多个媒体流
        int                         fTrackID;			
        int                         fPort;			//端口	
        UInt32                      fTimeScale;		//时间标度
        SimpleString                fTheTypeStr;	//"video" 或 "audio"
        SimpleString                fProtocolStr;	// "RTP/AVP"
        ArrayList<PayLoad>          fPayLoadTypes;	//PayLoad的数组列表, 说明fMediaStreamPtr有哪些PayLoad
        ArrayList<short>            fPayLoads;		//该数组包含所有的PayLoad的ID
...
class UDPSocketPair
{
	...
	//End *************  本地的RTP以及RTCP地址端口  *****************//

    BroadcasterSession *fBroadcasterSession;
    UInt8               fChannel;
    Bool16              fIsMultiCast;		//目标地址是否是组播地址
