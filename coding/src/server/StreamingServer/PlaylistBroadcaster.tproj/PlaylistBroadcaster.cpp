...
static void RegisterEventHandlers();  //设置控制台处理程序, 要讓控制台程序具有事件處理能力, 
...
*/
	...
	//bcastSetupFilePath = "C:\Program Files\VideoRelayServer\Playlists\untitled\untitled.config"
    if (argv[optind] != NULL)
    {
	...
	//1. bcastSetupFilePath = "C:\Program Files\VideoRelayServer\Playlists\untitled\untitled.config"
	//2. preflight = 0 ,			3. daemonize = 1 ,			4. showMovieList = 0, 
	//5. writeCurrentMovieFile =0,	6. destinationIP = NULL;	
	//7. writeNewSDP = 0,			8. errorlog = NULL;
...
//据 "C:\Program Files\VideoRelayServer\Playlists\untitled\untitled.config"返回
//   "C:\Program Files\VideoRelayServer\Playlists\untitled\"
char* GetBroadcastDirPath(const char * setupFilePath)
{
    ...
    return setupDirPath;
}//GetBroadcastDirPath(const char * setupFilePath)

//创建untitled.current以及untitled.upcoming
...
//确保以下的文件存在: 
//   1. untitled.stoplist, 2. untitled.replacelist, 3. untitled.insertlist, 4. untitled.upcomingfile
//   5. 更新picker, insertPicker, 以及sTempPicker
...
//   4. 以下部分更新picker以及sTempPicker
                // write upcoming playlist to .upcoming file 
    if (!::strcmp(broadcastParms->mShowUpcoming, "enabled"))	//如果 mShowUpcoming = "enabled"
    {
        //4.1 打开untitled.upcomingfile
		FILE *upcomingFile = fopen(broadcastParms->mUpcomingFile, "w");
        if(upcomingFile)
        {//以下34行代码
            sElementCount = 0;
		//1. 如果是随机播放, 则不显示即将播放的节目列表	
		...
		//2. 否则显示即将播放的节目
            {   qtss_fprintf(upcomingFile,"*PLAY-LIST*\n");
				...
				//2.1 如果picker中没有节目, 并且!picker->mStopFlag(???), 并且是顺序播放模式
				        //则重新读出节日表
                ...
                //2.2 如果是循环播放模式, 则sTempPicker读新从播放文件列表读取
...
//把当前要播放的文件写入untitled.current
//u=C:\Program Files\VideoRelayServer\Playlists\untitled\c:\Program Files\VideoRelayServer\Movies\\sample_100kbit.mov
...
//该函数由PlaylistBroadcaster的main函数调用, 主要实现视频文件的广播功能
//1.	尝试读name.config文件，确定是否有该文件，PreFlightSetupFile( bcastSetupFilePath )；
//2.	设置allocatedIPStr以及generateNewSDP
//3.	建立新的节目列表对象broadcastParms = new PLBroadcastDef(
//4. sSetupDirPath设置为"C:\Program Files\VideoRelayServer\Playlists\untitled\", 根据广播参数，创建picker对象，返回PlaylistPicker对象定义
//5. 定义BroadcastLog对象, 生成SDP
//6. 把broadcastDefPtr以及SDP的相关参数赋值给QTFileBroadcaster
//7. 创建untitled.pid文件,	untitled.current, untitled.upcoming等文件
static void PreFlightOrBroadcast( const char *bcastSetupFilePath, bool preflight, bool daemonize, bool showMovieList, bool writeCurrentMovieFile, char *destinationIP,bool writeNewSDP, const char* errorFilePath)
{
    ...
    RegisterEventHandlers();	//设置控制台处理程序, 要讓控制台程序具有事件處理能力, 
//1.	尝试读name.config文件，确定是否有该文件，PreFlightSetupFile( bcastSetupFilePath )；
...
//4. sSetupDirPath设置为"C:\Program Files\VideoRelayServer\Playlists\untitled\"
//    根据广播参数，创建picker对象，返回PlaylistPicker对象定义
    ...
    {   
        ...
        sTempPicker =  new PlaylistPicker(false);		//表示顺序选择节目, 不循环
        insertPicker = new PlaylistPicker(false);
        insertPicker->mRemoveFlag = true;				//表示不循环
    }
    ...
    {   
        char*   fileName;
        //"C:\Program Files\VideoRelayServer\Playlists\untitled\untitled.config"
        fileName = broadcastParms->mPlayListFile;
        // initial call uses empty string for path, NULL for loop detection list
        (void)PopulatePickerFromFile( picker, fileName, "", NULL );  ////该函数把中包含的move文件加入到picker链表中
...
//5. 定义BroadcastLog对象, 生成SDP
    // create the log file
    sgLogger = new BroadcastLog( broadcastParms, &sSetupDirPath );	//记录和Broadcast相关的日志, 包括错误日志, 数据日志, 信息日志
...
//8. 循环取出节目, 并播放该节目
    ...
    {//以下150行代码
		...
		//8.1  获取节目列表中的一个节目, 首先从insertPicker获得节目, 如果失败, 则从picker获得节目
        ...
        {//以下代码120行
            ...
            if(!preflight)  //是否重新取胜播放文件
            {   UpdateCurrentFile(broadcastParms, thePick);
                
                /* 如果播放文件已经用完了, 则重新取用播放文件if playlist is about to run out repopulate it */
            ...
            { //以下为 40行代码
                ...
                SInt64 endTime = 0;
                playError = fileBroadcaster.PlayMovie( thePick, broadcastParms->mCurrentFile );  //broadcastParms->mCurrentFile = untitled.current
...
// 按照broadcastParms参数的不同, 创建不同参数的PlaylistPicker对象, 并返回该对象
...
//  PLBroadcastDef *broadcastParms, 	//广播列表参数，解析配置文件，读入相关参数, 见[4.3.3.1.1]
//  bool preflight, 					//
//  bool overWriteSDP, 				//是否辐覆盖SDP文件
//  bool generateNewSDP, 				//是否生成新的SDP文件
//  int* numErrorsPtr, 					//错误数目
//  char* refMovie					//影视的引用
static bool DoSDPGen( PLBroadcastDef *broadcastParms, bool preflight, bool overWriteSDP, bool generateNewSDP, int* numErrorsPtr, char* refMovie)
{
    ...
    {
//1. 新建SDPGen对象SDPGen* sdpGen = new SDPGen;
...
//该函数打印一个节点的元素名, 并把该元素名打印到*file文件中
static void PrintPlaylistElement(PLDoubleLinkedListNode<SimplePlayListElement> *node,void *file)
{   
    sElementCount ++;
    if (sElementCount <= sMaxUpcomingListSize)  
...
//该函数在file文件中打印播放列表, 打印过程调用PrintPlaylistElement(...)函数实现
static void ShowPlaylistElements(PlaylistPicker *picker,FILE *file)
{
    if (sElementCount > sMaxUpcomingListSize)   
        return;
...
/* ***************************************************** */
//设置控制台处理程序, 要讓控制台程序具有事件處理能力, 
static void RegisterEventHandlers()
{
#ifdef  __Win32__
    SetConsoleCtrlHandler( (PHANDLER_ROUTINE) SignalEventHandler, true);	
	//设置控制台处理程序, 要讓控制台程序具有事件處理能力, SignalEventHandler相当于Win32 GUI程序中的窗口處理函數。這個函數的原型如下：
 		//BOOL WINAPI HandlerRoutine(
		//	DWORD dwCtrlType   		//  control signal type, dwCtrlType具有预定义的几种类型, 详细见源代码分析
		//);
    return;
#endif
...
/* ========================================================================
 * Signal and error handler.该函数为控制台处理程序, 有消息时进行相应的处理
 */
static void SignalEventHandler( int signalID )
{   
