...
//fname = "C:\Program Files\VideoRelayServer\Playlists\untitled\untitled.playlist"
//该函数把中包含的move文件加入到picker链表中, untitled.playlist可能包含嵌套, 为了检测循环嵌套, 定义了一个双向链表对象
int PopulatePickerFromFile( PlaylistPicker* picker, char* fname, const char* basePath, LoopDetectionList *ldList )
{
    ...
    if ( !pickErr )
    {
        ldElement = new LoopDetectionListElement( path );	//只是定义一个字符串对象
        
        Assert( ldElement );
        
        if ( ldElement )
        {   ldNode = new LoopDetectionNode( ldElement );	//把字符串封装成结点
            Assert( ldNode );
            if ( !ldNode )
                pickErr = kPickerPopulateNoMem;
        }
    ...
    if ( !pickErr )
    {
        ...
        //删除文件名部分, 只留下文件路径, 从"C:\Program Files\VideoRelayServer\Playlists\untitled\untitled.playlist"删除untitled.playlist
		//
        ...
        if ( ::fgets( lineBuff, lineBuffSize, weightings ) != NULL )		//char *fgets( char *string, int n, FILE *stream );

        {
            ...
            if ( 0 != ::strncmp(thisLine,"*PLAY-LIST*",11) )  //untitled.playlist文件的第一行必须是*PLAY-LIST*
            {   
...
//                qtss_printf("line = %s\n", lineBuff);
                ...
                else if (*next == '+') // a list, 如果以+开始, 表示是一个列表, 则进行循环
                {
                ...
                else if ( *next )	//反之
                {
                    ...
                    if ( *next == '"' )
                        next = ::GetQuotedWord( wordBuff, next, wordBuffSize );//读出路径"c:\Program Files\VideoRelayServer\Movies\\sample_100kbit.mov"
                    else
                        next = ::GetWord( wordBuff, next, wordBuffSize );
                    ...
                    if (*wordBuff)
                    {
                        ...
                        {   // it's an absolute path..
                            ::strcpy( expandedFileName, wordBuff );  //复制"c:\Program Files\VideoRelayServer\Movies\\sample_100kbit.mov"
                        }
                        
                        // then get the weighting ( if supplied )
                        next = ::GetWord( numBuff, next, 32 );       //获得weight
