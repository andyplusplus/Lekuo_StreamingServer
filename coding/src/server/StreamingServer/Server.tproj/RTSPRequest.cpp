...
//主要解析第一行, DESCRIBE rtsp://159.99.242.108/sample_100kbit.mov RTSP/1.0
QTSS_Error RTSPRequest::ParseFirstLine(StringParser &parser)
{   
    //first get the method，如GET等
    StrPtrLen theParsedData;		//设置为待解析的数据
    ...
    fMethod = RTSPProtocol::GetMethod(theParsedData);			//返回值为QTSS_RTSPMethod的枚举类型 0 
    ...
    //now parse the uri
    QTSS_Error err = ParseURI(parser);			//RTSP://...
    if (err != QTSS_NoErr)
        return err;
...
QTSS_Error RTSPRequest::ParseHeaders(StringParser& parser)
{
    ...
    while ((parser.PeekFast() != '\r') && (parser.PeekFast() != '\n')) 
    {
        ...
        isStreamOK = parser.GetThru(&theKeyWord, ':');		//获得冒号以前的字符串
        if (!isStreamOK)
...
//解析Transport: RTP/AVP;unicast;client_port=6974-6975
