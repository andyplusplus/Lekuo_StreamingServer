...
SInt64 RTSPSession::Run()
{
    ...
    while (this->IsLiveSession())
    {
        ...
        //
	#if _WZD_RTSP_
		QTSSLogging::RTSPSessionRunLog(this, fState, NULL, NULL);
	#endif //_WZD_RTSP_
        switch (fState)
        {
            case kReadingFirstRequest:
            {
                ...
                {
                    // 如果返回QTSS_NoErr, 表明我们已经从socket读得所有数据，但还没有完整的请求，等待更多的数据
					// If the RequestStream returns QTSS_NoErr, it means that we've read all outstanding data off the socket,
                ...
                // 如果返回E2BIG，表明缓存已满。此时，我们进入下一状态
				// If we get an E2BIG, it means our buffer was overfilled. In that case, we can just jump into the following state, and
            ...
            {   
                ...
                fState = kHaveNonTunnelMessage; // assume it's not a tunnel setup message prefilter will set correct tunnel state if it is.
            ...
            {   
                ...
                // We have an RTSP request and are about to begin processing. We need to make sure that anyone sending 
				// interleaved data on this session won't be allowed to do so until we are done sending our response
                // We also make sure that a POST session can't snarf in while we're processing the request.
                fReadMutex.Lock();
                fSessionMutex.Lock();
                
                // The fOutputStream's fBytesWritten counter is used to count the # of bytes for this RTSP response. So, at
                // this point, reset it to 0 (we can then just let it increment until the next request comes in)
            ...
            case kFilteringRequest:
            {
				...
				if (fSentOptionsRequest && this->ParseOptionsResponse())		//判断是否是RTSP请求
				{
...
//确定qtssRTSPReqFullRequest的前四位是否是"RTSP"
...
//解析请求函数
//1. 解析解析请求 fRequest->Parse()
//2. 获得fRTPSession，如果没有fRTPSession，新建一个fRTPSession
//3. 处理请求的方法
//4. 设置参数fRoleParams.rtspRequestParams.inClientSession = fRTPSession
...
void RTSPSession::CleanupRequest()
{
    if (fRTPSession != NULL)
    {
        // Release the ref.
        OSRefTable* theMap = QTSServerInterface::GetServer()->GetRTPSessionMap();
        theMap->Release(fRTPSession->GetRef());		//从任务列表中删除任务
    ...
    if (fRequest != NULL)		//删除RTSPRequest
    {
...
QTSS_Error  RTSPSession::CreateNewRTPSession(OSRefTable* inRefTable)
{
    ...
    
    {
        ...
        while (activationError == EPERM)		//通过while函数生成一个SessionID，直到找到不重复的ID为止
        {
    ...
    // make sure to resolve the RTPSession object out of the map, even though we don't actually need to pointer.
    OSRef* theRef = inRefTable->Resolve(&fLastRTPSessionIDPtr);
    Assert(theRef != NULL);
...
void RTSPSession::SetupClientSessionAttrs()
{
    ...
    tempStr.Len = 0;
    (void) this->GetValuePtr(qtssRTSPSesLocalDNS, (UInt32) 0, (void **) &tempStr.Ptr, &tempStr.Len);//qtssRTSPSesLocalDNS 与当前RTSP会话的本地IP地址相关联的DNS名称。
    Assert(tempStr.Len != 0);   
...
//把授权信息保存到RTSPSession以及RTPSession中
