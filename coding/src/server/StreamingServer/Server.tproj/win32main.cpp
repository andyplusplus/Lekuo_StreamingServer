...
{ 
	...
	//	_WZD_PATH_ Start
		//char* theConfigFilePath = "c:\\Program Files\\VideoRelayServer\\streamingserver.cfg";
		//char* theXMLFilePath = "c:\\Program Files\\VideoRelayServer\\streamingserver.xml";
		//
		DEFAULTPATHS_Initialize();
		char* &theConfigFilePath = DEFAULTPATHS_ConfigFileName;		//引用的作用是给一个变量起一个别名，例如有一个变量a，想给他起一个别名b，可以写成：int a;    int   &b=a; 
		char* &theXMLFilePath = DEFAULTPATHS_XMLFilePathName;		//      这样声明后，代表a和b就是同一变量单元，而&不是代表取地址符号，而是引用声明符，说明定义了b可并没有为它另开辟内存单元，b和a就是同一变量。

//		&theConfigFilePath = &DEFAULTPATHS_ConfigFileName
//		&theXMLFilePath= &DEFAULTPATHS_XMLFilePathName;
	//	_WZD_PATH_ End
   ...
   char* compileType = "";
    ...
    while ((ch = getopt(argc,argv, "vdxp:o:c:irsS:I")) != EOF) // opt: means requires option
    {
        switch(ch)
        {
            case 'v':
                ...
                qtss_printf("-i: Install the VideoRelayServer service\n");
                qtss_printf("-r: Remove the VideoRelayServer service\n");
                qtss_printf("-s: Start the VideoRelayServer service\n");
            ...
            case 'x':
                ...
                break;
            case 'i':
                qtss_printf("Installing the VideoRelayServer service...\n");
                ::InstallService("VideoRelayServer");
                qtss_printf("Starting the VideoRelayServer service...\n");
                ::RunAsService("VideoRelayServer");
                ::exit(0);
                break;
            case 'r':
                qtss_printf("Removing the VideoRelayServer service...\n");
                ::RemoveService("VideoRelayServer");
                ::exit(0);
            case 's':
                qtss_printf("Starting the VideoRelayServer service...\n");
                ::RunAsService("VideoRelayServer");
    ...
    sXMLParser = new XMLPrefsParser(theXMLFilePath);  //把路径设置为fFile, 并打开文件, 设置文件长度和日期  c:\\Program Files\\lekuo\\StreamingServer\\streamingserver.xml
    ...
    if (!theXMLPrefsExist)
    {
        ...
        int prefsErr = sPrefsSource.InitFromConfigFile(theConfigFilePath);		//c:\\Program Files\\lekuo\\StreamingServer\\streamingserver.cfg
        if ( prefsErr )
    ...
    int xmlParseErr = sXMLParser->Parse();   //Streamingserver.xml
    if (xmlParseErr)
    {
	...
	//从配置文件解析sMessagesSource，此处"qtssmessages.txt"不存在
    sMessagesSource.InitFromConfigFile("qtssmessages.txt");

    //
    // Start Win32 DLLs
    WORD wsVersion = MAKEWORD(1, 1);	//加载该socket版本
    WSADATA wsData;
    (void)::WSAStartup(wsVersion, &wsData);
	//OSThread::DebugSleep(1000);//_WZD_tobeSleep_
    if (notAService)
    {
    ...
    qtss_printf("VideoRelayServer must either be started from the DOS Console\n");
    qtss_printf("using the -d command-line option, or using the Service Control Manager\n\n");
    qtss_printf("Waiting for the Service Control Manager to start VideoRelayServer...\n");
...
void InstallService(char* inServiceName)
{
    ...
    if (!theSCManager)
    {
        qtss_printf("Failed to install VideoRelayServer Service\n");
        return;
    }
    ...
    {
        ::CloseServiceHandle(theService);
        qtss_printf("Installed VideoRelayServer Service\n");
    }
    else
        qtss_printf("Failed to install VideoRelayServer Service\n");
...
void RemoveService(char *inServiceName)
{
    ...
    if (!theSCManager)
    {
        ...
        qtss_printf("Failed to remove VideoRelayServer Service, the Error = %lu\n", theError);
        return;
    }
    ...
    if (theService != NULL)
    {
        ...
        ::CloseServiceHandle(theService);
        qtss_printf("Removed VideoRelayServer Service\n");
    }
    else
        qtss_printf("Failed to remove VideoRelayServer Service\n");
