...
{   /*fields:   fAttrName, fFuncPtr, fAttrDataType, fAttrPermission，包含qtssModName, qtssModDesc, qtssModVersion, qtssModRoles, qtssModPrefs, qtssModAttributes, */
    ...
    /* 4 */ { "qtssModPrefs",           NULL,                   qtssAttrDataTypeQTSS_Object,qtssAttrModeRead | qtssAttrModePreempSafe  | qtssAttrModeInstanceAttrAllowed }, //模拟注册的角色
    ...
    fAttributes(NULL)
{
    ...
    fAttributes = NEW QTSSDictionary( NULL, &fAttributesMutex );	//新建字典对象
    
    this->SetVal(qtssModPrefs,      &fPrefs,            sizeof(fPrefs));		//服务器用以下函数构建字典属性，函数不做错误检查
...
QTSS_Error  QTSSModule::SetupModule(QTSS_CallbacksPtr inCallbacks, QTSS_MainEntryPointPtr inEntrypoint)
{
    ...
    if ((fFragment != NULL) && (inEntrypoint == NULL))
        theErr = this->LoadFromDisk(&inEntrypoint);		//自动从OSCodeFragment获取QTSS_MainEntryPointPtr
    ...
    //以文件模块为例，以下函数先调用QTSSFileModule_Main(void* inPrivateArgs)
	//QTSSFileModule_Main进一步调用_stublibrary_main(...)，其作用主要有二:
	//1. 设置QTSS_Private.cpp的sCallbacks = inCallbacks，inCallbacks即QTSServer::sCallbacks
	//2. 设置thePrivateArgs.outDispatchFunction = QTSSFileModuleDispatch
    theErr = (inEntrypoint)(&thePrivateArgs);	//调用模块的主入口点，对于dll, 主入口点由文件名生成，通过thePrivateArgs返回模块的分发函数
    if (theErr != QTSS_NoErr)
        return theErr;
    ...
    fDispatchFunc = thePrivateArgs.outDispatchFunction;	//分发流程
...
SInt64 QTSSModule::Run()
{
    ...
    //OSThreadDataSetter(void* inInitialValue, void* inFinalValue)，初始线程数据为fModuleState，退出该函数时，恢复线程数据NULL
 	OSThreadDataSetter theSetter(&fModuleState, NULL);		
    if (events & Task::kUpdateEvent)
    {   // force us to update to a new idle time 请求idleTime
    ...
    if (fRoleArray[kTimedIntervalRole])
    {
        if (events & Task::kIdleEvent || fModuleState.globalLockRequested)
        {
            ...
            if (fModuleState.globalLockRequested) // call this request back locked
                return this->CallLocked();    //调用Task的ForceSameThread();fWriteLock = true;return 0;
