...
QTSServerPrefs::PrefInfo QTSServerPrefs::sPrefInfo[] = //refer to QTSS.h 651 
{
    { kDontAllowMultipleValues, "0",        NULL                    },  //rtsp_timeout			0
    { kDontAllowMultipleValues, "180",      NULL                    },  //real_rtsp_timeout		1
	...
	{ kDontAllowMultipleValues,	DEFAULTPATHS_MOVIES_DIR, NULL       },	//movie_folder			5		**
	...
	{ kDontAllowMultipleValues,	DEFAULTPATHS_SSM_DIR,	NULL		},	//module_folder			12
	...
	//
	//	_WZD_PATH_ Start, 以下第二项用作占位符
	{ kDontAllowMultipleValues,"", NULL	},	//pid_file		
    ...
    fDisableThinning(false)
{
    ...
    //_WZD_PATH_ Start
	//从Default.CPP移过来，由于在Windows平台下该
	//char** strPrefsPidFile = GetPrefInfoDefaultValueAddr(qtssPrefsPidFile);
	//sprintf(*strPrefsPidFile,"%s%s%s",DEFAULTPATHS_PID_DIR, PLATFORM_SERVER_BIN_NAME, ".pid");
    //_WZD_PATH_ End
}
...
void QTSServerPrefs::SetupAttributes()
{
	...
	//WuZengDe Test
	char* theFolder = this->GetStringPref(qtssPrefsModuleFolder);
	delete [] theFolder;
}
