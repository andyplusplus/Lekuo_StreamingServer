...
QTSS_Error  RTPStream::Write(void* inBuffer, UInt32 inLen, UInt32* outLenWritten, UInt32 inFlags)
{
	...
	{
		//OutputDebugString("==>RTPStream::Write==>EAGAIN\n");
        return EAGAIN;
	}
    ...
    QTSS_PacketStruct* thePacket = (QTSS_PacketStruct*)inBuffer;	//把缓存区的数据转换为QTSS_PacketStruct结构的形式
    ...
    {   
		//OutputDebugString("==>RTPStream::  inFlags & qtssWriteFlagsIsRTCP\n");
		//
    ...
    else if (inFlags & qtssWriteFlagsIsRTP)
    {
        ...
        if (thePacket->suggestedWakeupTime > theTime)
        {
			//OutputDebugString("==>RTPStream::Write==>thePacket->suggestedWakeupTime > theTime\n");
            Assert(thePacket->suggestedWakeupTime >= fSession->GetOverbufferWindow()->GetSendInterval());
#if RTP_PACKET_RESENDER_DEBUGGING
		...
		//_WZD_DEBUG_with_Start
		//char thePrintChar[1024];
		//char* theBuff = thePrintChar;
		//sprintf(theBuff, "===========> RTPStream::Write(inLen = %d\n", inLen);
		//OutputDebugString(thePrintChar);
		//_WZD_DEBUG_with_End
		if(this->UpdateQualityLevel(thePacket->packetTransmitTime, theCurrentPacketDelay, theTime, inLen))			//2008.06.26 存在第一个包发不过去的情况，为了确保所有数据包都能发过去
        {
            if ( fTransportType == qtssRTPTransportTypeTCP )    // write out in interleave format on the RTSP TCP channel
...
void RTPStream::ProcessIncomingRTCPPacket(StrPtrLen* inPacket)
{
    ...
    {
        /* 由于RTCP包的可变长度特性, 以下处理有些不正常, 先作为通用的RTCP包来处理, 以断定实际包类型, 再以实际包的类型来处理
        ...
        */
        ...
        if (!rtcpPacket.ParsePacket((UInt8*)currentPtr.Ptr, currentPtr.Len))	//由StrPtrLen构造RTCPPacket
