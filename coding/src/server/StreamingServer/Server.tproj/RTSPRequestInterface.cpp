...
//"	初始化kRTSPRequestDictIndex字典对象, 以及初始化kRTSPHeaderDictIndex字典对象
...
void RTSPRequestInterface::WriteStandardHeaders()
{
    ...
    if (fStatus == qtssSuccessOK)
    {
        ...
        if (sendServerInfo)
        {   fOutputStream->Put(sPremadeHeaderPtr);		//放入标准的头结构RTSP/1.0 200 OK..Server: DSS/5.5.1 (Build/489.8; Platform/Win32; Release/Darwin; )..Cseq:
        }
        else
        {
