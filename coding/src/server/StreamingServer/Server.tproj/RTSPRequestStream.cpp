...
QTSS_Error RTSPRequestStream::ReadRequest()
{
    while (true)
    {
		...
		//在这种情况下，我们已经有该会话的请求，该请求处理已经完成，并移动到下一项。
		//我们首先需确认流中是否有滞留数据，如果有滞留数据，父会话把该数据认定为新的请求
        if (fRequestPtr != NULL)
        {
			...
			//数据往前移fRetreatBytes个字节
            if ((fRetreatBytes > 0) && (fRequest.Len > 0))
                ::memmove(fRequest.Ptr, fRequest.Ptr + fRequest.Len + fRetreatBytesRead, fRetreatBytes);

            // if we are decoding, we need to also move over the remaining encoded bytes to the right position in the fRequestBuffer
