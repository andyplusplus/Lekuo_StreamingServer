...
{   /*fields:   fAttrName, fFuncPtr, fAttrDataType, fAttrPermission */
    ...
    /* 3 */ { "qtssRTSPSesLocalDNS",        SetupParams,    qtssAttrDataTypeCharArray,  qtssAttrModeRead | qtssAttrModePreempSafe | qtssAttrModeCacheable },//qtssRTSPSesLocalDNS 与当前RTSP会话的本地IP地址相关联的DNS名称。
...
void    RTSPSessionInterface::Initialize()
{
	...
	for (unsigned int i = 0; i < kMaxRandomDataSize / sizeof(UInt32); i++)
		RTSPSessionInterface::sOptionsRequestBody[i] = ::rand();			//sOptionsRequestBody[kMaxRandomDataSize / sizeof(UInt32)]
...
void* RTSPSessionInterface::SetupParams(QTSSDictionary* inSession, UInt32* /*outLen*/)
{
    ...
    theSession->SetVal(qtssRTSPSesLocalDNS, theLocalDNSStr->Ptr, theLocalDNSStr->Len);//qtssRTSPSesLocalDNS 与当前RTSP会话的本地IP地址相关联的DNS名称。
    theSession->SetVal(qtssRTSPSesRemoteAddr, &theSession->fRemoteAddr, sizeof(theSession->fRemoteAddr));			//qtssRTSPSesRemoteAddr 客户端的IP地址。
