...
//解析请求头, 除第一行以外的所有信息都采用 XXX:value的格式
//如
//	Cseq: 1
//	Last-Modified: Tue, 01 Mar 2005 01:39:22 GMT
...
QTSS_RTSPHeader RTSPProtocol::GetRequestHeader(const StrPtrLen &inHeaderStr)
{
    ...
    //
    // Check to see whether this is one of our extension headers. These are very likely to appear in requests.
    ...
    //
    // It's not one of our extension headers, check to see if this is one of our normal VIP headers
    if ((theHeader != qtssIllegalHeader) &&
        (inHeaderStr.EqualIgnoreCase(sHeaders[theHeader].Ptr, sHeaders[theHeader].Len)))
        return theHeader;

    //
    //If this isn't one of our VIP headers, go through the remaining request headers, trying to find the right one.
