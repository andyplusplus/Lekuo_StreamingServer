...
{   /*fields:   fAttrName, fFuncPtr, fAttrDataType, fAttrPermission */
    ...
    /* 6  */ { "qtssSvrRTSPPorts",              NULL,   qtssAttrDataTypeUInt16,     qtssAttrModeRead },					//qtssSvrRTSPPorts 这是一个索引过的属性，含有服务器正在侦听的所有端口
    ...
    /* 9  */ { "qtssSvrIsOutOfDescriptors",     IsOutOfDescriptors,     qtssAttrDataTypeBool16, qtssAttrModeRead },		//qtssSvrIsOutOfDescriptors 当服务器用完了文件描述符时，这个属性为真；否则，这个属性为假。
    ...
    /* 12 */ { "qtssRTPSvrNumUDPSockets",       GetTotalUDPSockets,     qtssAttrDataTypeUInt32, qtssAttrModeRead },		//qtssRTPSvrNumUDPSockets 服务器当前正在使用的UDP套接口数。
    ...
    /* 20 */ { "qtssSvrHandledMethods",         NULL,   qtssAttrDataTypeUInt32,     qtssAttrModeRead | qtssAttrModeWrite | qtssAttrModePreempSafe  },//qtssSvrHandledMethods 服务器支持的方法。模块应该在QTSS_Initialize_Role角色中将自己支持的方法附加到这个属性上。
    /* 21 */ { "qtssSvrModuleObjects",          NULL,   qtssAttrDataTypeQTSS_Object,qtssAttrModeRead | qtssAttrModePreempSafe },//qtssSvrModuleObjects 模块对象，代表每个模块。
    ...
    /* 27 */ { "qtssSvrClientSessions",         NULL,   qtssAttrDataTypeQTSS_Object,qtssAttrModeRead },			//qtssSvrClientSessions 一个对象，包含存储在索引过的QTSS_ClientSessionObject对象中的所有客户会话。
    /* 28 */ { "qtssSvrCurrentTimeMilliseconds",CurrentUnixTimeMilli,   qtssAttrDataTypeTimeVal,qtssAttrModeRead},//qtssSvrCurrentTimeMilliseconds 服务器当前的时间，以毫秒计。获取这个属性的值等同于调用//QTSS_Milliseconds函数。
...
//
//"	初始化kServerDictIndex字典对象, 用来存贮服务器的一些参数
//"	初始化kQTSSConnectedUserDictIndex字典对象
//"	通过StringFormatter初始化sServerHeaderPtr为"Server: DSS/5.5.1 (Build/4898; Platform/ Win32; Release/Darwin;)"
void    QTSServerInterface::Initialize()
{
    ...
    Assert(sServerHeaderPtr.Len < kMaxServerHeaderLen);  //sServerHeaderPtr为"Server: DSS/5.5.1 (Build/4898; Platform/ Win32; Release/Darwin;)"
}
    ...
    fNumThinned(0)
{
	...
	//以下值保存在Ditctionary字典的数组中，通过Set函数把枚举类型和对象指针建立联系，以便于下次直接通过枚举获得对象
	//Ditctionary字典的数组在QTSSDictionaryMap::Inititize时建立
...
SInt64 RTPStatsUpdaterTask::Run()
{
    ...
    //
	//首先更新总的字节数, 由于总字节数是64比特数, 所以没有可用的自动函数
    //
