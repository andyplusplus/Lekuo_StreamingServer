                                            ...
                                            Bool16 isInternal)
{
    ...
    // 获取缓存及长度
    char* theBuffer = theAttrs[theMapIndex].fAttributeData.Ptr;		//theBuffer指向一个缓存，可能是同一属性的多个值
    *outValueLen = theAttrs[theMapIndex].fAttributeData.Len;		//缓存的大小
    
	Bool16 cacheable = theMap->IsCacheable(theMapIndex);
	//如果属性函数为NULL，且
	//    可缓存&&*outValueLen=0，或不可缓存
	if ( (theMap->GetAttrFunction(theMapIndex) != NULL) && ((cacheable && (*outValueLen == 0)) || !cacheable) )  //不做缓存处理
    {
        //如果设置了属性retrieval函数
		//		如果不能缓存，或
		//		能缓存，但没有赋值
        ...
        //如果参数获取函数没有返回属性值，则重新从数组获得该值，以防止函数修改该值
    ...
    //如果有数据，??这里有问题
    theBuffer += theAttrs[theMapIndex].fAttributeData.Len * inIndex;   //fAttributeData为StrPtrLen类型
                                        ...
                                        QTSSDictionary** newObject, QTSSDictionaryMap* inMap, UInt32 inFlags)
{
    ...
    *outIndex = numValues;  // 把对象增加到下一个位置
    ...
    QTSS_Error err = SetValue(inAttrID, *outIndex, &dict, len, inFlags);		//把对象加到特定位置上
    if (err != QTSS_NoErr)
    {
        delete dict;
        return err;
                                        ...
                                        UInt32 inFlags)
{
    ...
    if (dataType == qtssAttrDataTypeCharArray)  //如果是字符串类型
    {
        if (inIndex > 0)
                                                    ...
                                                    QTSS_AttrPermission inPermission )
{
    ...
    //
    // 判断是否在静态map中存在该属性，如果存在，我们不能增加该实例属性，返回错误
...
//申请空间建立所有静态/动态字典对象, 并为静态的字典对象的属性申请存贮空间
void QTSSDictionaryMap::Initialize()
{
    //
    // Have to do this one first because this dict map is used by all the other dict maps.
	// 必须先执行该初始化操作, 因为该QTSSDictionaryMap初所有其它QTSSDictionaryMap使用
	//
	// 构造所有静态字典对象Map, 并按照其属性数(每个Map的属性预定好)分配指针的存贮空间，  kIllegalDictionary = kNumDynamicDictionaryTypes + kNumDictionaries
     sDictionaryMaps[kAttrInfoDictIndex]     = new QTSSDictionaryMap(qtssAttrInfoNumParams);
	
    // Setup the Attr Info attributes before constructing any other dictionaries，//字典对象的属性在该字典对象的拥有者构造时构造
	...
	//
	//构造其它字典对象，现在虽然字典对象已经构造好了，但字典对象的属性还没有构造好
	//字典对象的属性在该字典对象的拥有者构造时构造
	//
...
:   fNextAvailableID(inNumReservedAttrs), fNumValidAttrs(inNumReservedAttrs),fAttrArraySize(inNumReservedAttrs), fFlags(inFlags)
{
    ...
    fAttrArray = NEW QTSSAttrInfoDict*[fAttrArraySize];			//该对象在QTSSDictionaryMap中定义属于父类
...
QTSS_Error  QTSSDictionaryMap::GetAttrInfoByIndex(UInt32 inIndex, QTSSAttrInfoDict** outAttrInfoObject)
{
    ...
    if (fFlags & kAllowRemoval)
    {
        ...
        actualIndex = 0;
        for (UInt32 x = 0; x < max; x++)//如果允许移去，则直接索引不一定正确
